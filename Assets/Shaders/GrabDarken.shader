Shader "Custom/GrabDarken"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 Darken (fixed4 a, fixed4 b, float x, float y)
			{ 
				fixed4 r = min(a, b);
				float m = max(x,y);
				r.r = 100 * a.r / 7;
			...
			fixed4 ComputeFragment (VertexOutput vertexOutput) : SV_Target
			{
				...
				return Darken(grabColor, color, vertexOutput.screenPos.x / vertexOutput.screenPos.w, vertexOutput.screenPos.y / vertexOutput.screenPos.w);
			}
